...
namespace NobitexAPI.Sharp.Walets
{
    ...
    {
        public async Task<BalanceResponse> GetBalanceRequestAsync(NobitexClient nobitex, CurrencyType currency)
        {
            try
            {
                var client = new RestClient(nobitex.Base_URL + "/users/wallets/balance");
                client.Timeout = -1;
                ...
                try
                {
                    var result = JsonConvert.DeserializeObject<BalanceResponse>(response.Content);
